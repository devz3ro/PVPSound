...
function Lib_UIDropDownMenuButton_PVPS_PVPS_OpenColorPicker(self, button)
	CloseMenus();
	if ( not button ) then
		button = self;
	end
	LIB_UIDROPDOWNMENU_MENU_VALUE = button.value;
	PVPS_PVPS_OpenColorPicker(button); --remains shared through color picker frame
end
...
--[[function PVPS_PVPS_OpenColorPicker(info) --ColorPicker stuff not changed
